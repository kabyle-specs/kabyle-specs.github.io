...
| Rétrotraduction | Les caractères spécifiques distincts sont réversibles dans le profil kabyle. Les séquences partagées avec des symboles CBFU, ainsi que les ambiguïtés internes de la ponctuation CBFU, restent dépendantes du contexte de profil ou d'une heuristique explicite documentée. |
...
| 0.3.1-draft | Ajout d'une note informative sur les ambiguïtés de rétrotraduction de la ponctuation CBFU (`?`/`(`, `!`/`+`, `"`/`«`/`»`/`=`, `-`/`–`/`—`) et précision du critère de conformité sur la rétrotraduction. |
